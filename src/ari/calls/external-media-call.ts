...
export async function externalMediaCall(ari: Client, channel: Channel) {
  ...
  function closeAllResources() {
    if (bridge) bridge.destroy().catch((err) => Logger.error(`Destruindo bridge ${bridge.id}`, err.message, 'RouterCallAppService'));
    if (channel) channel.hangup().catch((err) => Logger.error(`Desligando canal ${channel.name}`, err.message, 'RouterCallAppService'));
    if (externalMediaChannel)
      externalMediaChannel.hangup().catch((err) => Logger.error(`Desligando canal ${externalMediaChannel.name}`, err.message, 'RouterCallAppService'));
  }
  ...
  bridge.addChannel({ channel: channel.id }).catch((err) => Logger.error(`Adicionando canal ${channel.name} ao bridge`, err.message, 'RouterCallAppService'));

  const externalMediaChannel = ari.Channel();

  externalMediaChannel.on('StasisStart', (stasisStart, externalMediaChannel) => {
    bridge
      .addChannel({ channel: externalMediaChannel.id })
